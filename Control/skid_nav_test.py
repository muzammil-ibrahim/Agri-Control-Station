...
from dronekit import connect, LocationGlobalRelative  # type: ignore[import-not-found]
...
def load_waypoints_from_csv(csv_path: Path, default_alt: float = 0.0):
    waypoints = []
    ...
    with csv_path.open(newline="") as fp:
        ...
        for idx, row in enumerate(reader, start=1):
            ...
            alt_raw = row.get("alt") or row.get("altitude")

            lat = float(lat_raw)
            lon = float(lon_raw)
            heading = float(heading_raw) if heading_raw not in (None, "") else 0.0
            alt = float(alt_raw) if alt_raw not in (None, "") else default_alt
...
def run_mission(robot: SkidRobot, waypoints, stop_time: float):
    ...
    for index, (lat, lon, alt, heading) in enumerate(waypoints, start=1):
        destination = LocationGlobalRelative(lat, lon, alt)

        print(f"\nWaypoint {index}: lat={lat}, lon={lon}, alt={alt}, heading={heading}")
...
def parse_args():
    ...
    parser.add_argument(
        "--waypoints",
        default=str(Path(__file__).resolve().parents[1] / "Backend" / "test" / "received_waypoints.csv"),
        help="CSV path with waypoint columns (lat/lon or latitude/longitude)",
    )
    parser.add_argument("--vehicle-port", default="COM3", help="Pixhawk telemetry port")
    parser.add_argument("--vehicle-baud", type=int, default=57600, help="Pixhawk baud rate")
    parser.add_argument("--motor-port", default="COM4", help="Skid motor controller port")
