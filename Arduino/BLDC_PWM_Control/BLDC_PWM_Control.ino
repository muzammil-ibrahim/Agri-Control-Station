//  this code is for controlling a BLDC motor using an Arduino UNO.
// It uses the ArduinoJson library to parse JSON commands received over the serial interface.
// The motor can be controlled to move forward, backward, or stop based on the received commands
//  this setup requires the DAC module to send pure analog voltage to the motor controller for speed control. 
// The DAC module is connected to the Arduino via I2C and can output a voltage range of 0-5V, which is used to control the speed of the BLDC motor. 
// The direction of the motor is controlled by a digital pin(Relay Switch), and the motor can be stopped by setting the speed to zero.
